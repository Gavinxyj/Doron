...
/*
	...
	LOG4CXX_INFO(m_pDriverCtrl->logger,"tickTime = "<< tickTime << " m_tickTime = " << m_tickTime << " tickTime - m_tickTime = "<< tickTime - m_tickTime );
	m_tickTime = tickTime;
...
*/
	...
	*/
	...
	LOG4CXX_INFO(m_pDriverCtrl->logger,"tickTime");
	...
	{
		CString strRet;
		strRet.Format("ip = %s|windowId = %d", "null", this->m_curIndex);
		m_pDriverCtrl->IPADDRESS(strRet);
	}
	for (int nLoop = 0; nLoop < MAX_CHILDWND; nLoop ++)
	{
...
int CDlgOutput::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_digitalZoom.ShowWindow(SW_SHOW);
	...
	m_playBackBtn.ShowWindow(SW_SHOW);
...
{
	LOG4CXX_DEBUG(m_pDriverCtrl->logger,"OnLButtonDblClk我被执行了");
