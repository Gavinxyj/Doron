...
#include "../HIKVISION/HkInterfaceImpl.h"
#include "../DHVISION/DhInterfaceImpl.h"
...
SHORT CDriverAdaptorPlatfromCtrl::OpenConnection(LPCTSTR strIp, ULONG dwPort, LPCTSTR userName, LPCTSTR password, LPCTSTR DeviceType, LPCTSTR params)
{
	...
	CString windowId = "0";
	CString cameraId = "0";
	LOG4CXX_INFO(logger,"params = " << params);
	if ( NULL != params )
	{
	...
	{
	//	pDriverAdaptorPat = new CHk8600InterfaceImpl();
...
SHORT CDriverAdaptorPlatfromCtrl::StartRealPlay(LPCTSTR params)
{
	...
	if (m_mapFactory.Lookup(windowId,pParams))
	{
		...
		bRet = pParams->pDriverAdaptorPat->StartRealPlay(&m_pDlgOutPut[atoi(windowId)]->m_static);
...
SHORT CDriverAdaptorPlatfromCtrl::splitScreen(SHORT dwScreenNum)
{
	...
	for (int nLoop = 0; nLoop < dwScreenNum; nLoop ++)
	{
		...
		m_pDlgOutPut[nLoop]->m_static.MoveWindow(0,25,nWidth,nHight - 25);
		if (nWidth < 200)
		{
...
int CDriverAdaptorPlatfromCtrl::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_pDlgOutPut[0]->m_static.MoveWindow(0,25,rect.Width(),rect.Height() - 25);
...
void CDriverAdaptorPlatfromCtrl::OnLButtonDown(UINT nFlags, CPoint point)
{
...
void CDriverAdaptorPlatfromCtrl::OnLButtonDblClk(UINT nFlags, CPoint point)
{
	// TODO: 在此添加消息处理程序代码和/或调用默认值
//	MessageBox("dbdown");
